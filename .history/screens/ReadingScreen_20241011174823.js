...
export default function ReadingScreen({ route }) {
  const { book, chap, verse } = route.params;
  ...
  useEffect(() => {
    if(verseRefs.current[verse - 1]){
      verseRefs.current[verse - 1].measureLayout(
        ...
        <Text style={styles.mainContent}>
          {Object.entries(bookText).map(([key, value]) => (
            <Text
              ...
              ref={(el) => (verseRefs.current[parseInt(key) - 1] = el)}
            >
