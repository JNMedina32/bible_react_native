...
export default function BibleBookSelectionScreen({ navigation }) {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View>
        <PillButton text={"Old Testament"} onPress={() => handleSelection("Old Testament")} />
        <PillButton text={"New Testament"} onPress={() => handleSelection("New Testament")} />
      </View>
      <Animated.View style={[styles.dropdownContainer, { height: dropdownAnim.interpolate({
        inputRange: [0, 1],
        outputRange: [0, 200],
      }) }]}>
        <FlatList
