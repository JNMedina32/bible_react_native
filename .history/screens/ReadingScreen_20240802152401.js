...
export default function ReadingScreen({ route, navigation }) {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <Text style={[styles.header, {color: colors.text,    fontSize: 23,
}]}>{book}</Text>
      <ScrollView>
        <Text style={[styles.chapter, {color: colors.text}]}>{chapter}</Text>
        <Text style={[styles.mainContent, {color: colors.text}]}>{bookText}</Text>
      </ScrollView>
