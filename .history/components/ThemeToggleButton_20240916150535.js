...
export default function ThemeToggleButton() {
  const { theme, colorMode } = useGlobalState();
  const dispatch = useGlobalDispatch();
  const { colors } = theme;

  const animation = useRef(new Animated.Value(colorMode ? 1 : 0)).current;

  useEffect(() => {
    Animated.timing(animation, {
      toValue: colorMode ? 1 : 0,
      duration: 300,
  ...
  const toggleSwitch = () => {
    dispatch({ type: "TOGGLE_THEME", payload: colorMode === "light" ? "dark" : "light" });
  };
