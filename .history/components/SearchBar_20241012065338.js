...
import { getBibleBooks, getUserSearch, getVerses } from "../services/readQueries";
...
export default function SearchBar({ placeholder, navigation }) {
  ...
  const handleSearch = (searchItem) => {
    ...
    if (searchObj.type === "reference") {
      if(searchObj.verseStart === null) {
        navigationHandler("ReadingScreen", {
          book: searchObj.bookame,
          chap: searchObj.chapter,
        });
      navigationHandler("SearchResultsScreen", {
        book: searchObj.bookName,
        chap: searchObj.chapter,
        verse: searchObj.verse,
      });
    } else {
  ...
  return (
    <View style={styles.container}>
      ...
      {predictiveSearch.length > 0 && (
        <ScrollView
          style={[styles.suggestionList, { borderColor: colors.tertiary }]}
        >
            ...
            >
              <View style={{ flex: 1, justifyItems: "center" }}>
                <Text
                  style={[
                    styles.listText,
                    { color: colors.text, borderColor: colors.tertiary },
                  ]}
                >
                  {book.book_name}
                </Text>
