...
async function getVerses(
  ...
  if (tableName) {
    try {
      const result = await db.getAllAsync(
        `SELECT book_name, chapter, verse, text FROM ${tableName} WHERE book_name = ? AND chapter = ? AND verse BETWEEN ? AND ?;`,
        [book_name, chapter, fromVerse, toVerse]
      );
      let verseObj = {};
      for (const book_name of result) {
        verseObj = { ...verseObj, book_name: book_name.book_name, chapter };
      }
      return result;
    } catch (error) {
