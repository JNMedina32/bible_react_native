...
async function getVerses(
  ...
  if (tableName) {
    try {
      const result = await db.getAllAsync(
        `SELECT book_name, chapter, verse, text FROM ${tableName} WHERE book_name = ? AND chapter = ? AND verse BETWEEN ? AND ? ORDER BY verse DESC;`,
      ...
      console.log("getVerses results: ", verseObj);
      return result;
    } catch (error) {
