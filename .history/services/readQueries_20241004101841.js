...
async function getChapters(db, book_nsmr, chapter, translation, func) {
  const tableName = translationTableMap[translation];
  ...
  if (tableName) {
    ...
    let verseObj = {};
    for (const book_nsmr of result) {
      verseObj = { ...verseObj, [book_nsmr.verse]: book_nsmr.text };
    }
    func(verseObj);
  } else {
...
async function getNumOfChap(db, book_nsmr, translation, func) {
  const tableName = translationTableMap[translation];
